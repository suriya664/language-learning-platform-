...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (navToggle && navMenu && navButtons) {
        // Function to position buttons below menu
        function positionButtonsBelowMenu() {
            if (navMenu.classList.contains('active') && navButtons.classList.contains('active')) {
                // Remove bottom border and padding from menu when buttons are below
                ...
                // Position buttons below menu
                navButtons.style.top = (navbarHeight + menuHeight) + 'px';
            } else {
                // Restore menu styling when buttons are not active
        ...
        function toggleMenu(e) {
            ...
            if (navMenu.classList.contains('active')) {
                ...
                // Position buttons below menu after a short delay to ensure menu is rendered
                setTimeout(positionButtonsBelowMenu, 10);
            } else {
        ...
        window.addEventListener('resize', function() {
            ...
            resizeTimeout = setTimeout(function() {
                if (navMenu.classList.contains('active') && navButtons.classList.contains('active')) {
                    positionButtonsBelowMenu();
                }
            }, 100);
        });

        // Add click event (works for mouse)
        navToggle.addEventListener('click', toggleMenu);
        
        // Add touch event (works for touch devices)
        ...
        navBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (btn.classList.contains('nav-btn-login')) {
                    window.location.href = 'login.html';
                } else if (btn.classList.contains('nav-btn-register')) {
        ...
        document.addEventListener('touchend', function(e) {
            if (navMenu.classList.contains('active')) {
                const isTouchInside = navMenu.contains(e.target) || 
                                     navButtons.contains(e.target) || 
                                     navToggle.contains(e.target);
